...
from src.steamworks.dylib_manager import setup_dylib, validate_dylib
...
console = Console()
logger = logging.getLogger("steam_unlock")
...
def _run_app(args: argparse.Namespace) -> int:
    ...
    if not pre.ok:
        ...
        logger.error(
            "preflight_failed",
            extra={
                "platform_ok": pre.platform_ok,
                "python_ok": pre.python_ok,
                "steam_running": pre.steam_running,
                "errors": pre.errors,
            },
        )
        if not pre.steam_running and pre.platform_ok and pre.python_ok:
            raise SteamNotRunningError(pre.errors[0] if pre.errors else "Steam not running")
        raise ConfigError(pre.errors[0] if pre.errors else "Preflight failed")

    console.print("[green]✓[/green] Preflight: macOS · Python · Steam")
    logger.info("preflight_ok", extra={"steam_running": pre.steam_running})
    ...
    if LOCAL_DYLIB.exists():
        ...
        if ok:
            ...
            console.print(
                f"[yellow]![/yellow] Steamworks SDK: невалидная локальная копия — {reason}"
            )
            try:
...
def run(argv: list[str] | None = None) -> int:
    ...
    except SteamNotRunningError as e:
        console.print(f"[red]Steam не запущен:[/red] {e}")
