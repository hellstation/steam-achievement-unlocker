...
logger = logging.getLogger("steam_unlock.steamworks.ctypes")
...
class SteamworksCtypes:
    ...
    def init(self, app_id: int, dylib_path: Path) -> tuple:
        ...
        try:
            ...
            logger.exception(
                "steamworks_request_current_stats_failed",
                extra={"app_id": app_id},
            )
            self.shutdown()
            return False, f"Ошибка RequestCurrentStats: {e}"
