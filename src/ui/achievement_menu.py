...
from ..preflight import is_steam_running
...
console = Console()
logger = logging.getLogger("steam_unlock.ui.achievement_menu")
...
def apply_achievements(api: SteamWebAPI, game: dict, achievements: list, action: str):
    ...
    if not is_steam_running():
        console.print(
            "[red]Steam не запущен. Откройте Steam, войдите в аккаунт и повторите.[/red]"
        )
        logger.error("apply_achievements_steam_not_running", extra={"app_id": app_id})
        return
    ...
    try:
        ...
        if not ok:
            logger.error(
                "apply_achievements_steamworks_init_failed",
                extra={"app_id": app_id, "dylib": str(dylib), "info": info},
            )
            console.print(f"[red]✗ Ошибка:[/red]\n{info}")
            return

        logger.info(
            "apply_achievements_started",
            extra={"app_id": app_id, "action": action, "target_count": len(targets), "dylib": str(dylib)},
        )
    ...
    if success:
        console.print(f"[green]✓ Успешно: {success}[/green]")
    if failed:
